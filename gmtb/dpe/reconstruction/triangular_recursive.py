...
def triangular_recursive(object_vector, embedding, object_set, dist_func, weighted_mean_func):
    ...
    while len(object_set) > 1:
        ...
        for i in range(0, len(object_set)-2, 3):
            ...
            # reconstruct object between first and second
            ...
            emb = alpha * embedding[ind[i]] + (1-alpha) * embedding[ind[i+1]]

            # reconstruct object to the third
            alpha = compute_alpha(emb, embedding[ind[i + 2]], object_vector)
            nm, bv = best_weighted_mean(nm, object_set[ind[i + 2]], alpha, orig_set,
                                        dist_func, weighted_mean_func)

            new_object_set.append(nm)
            new_embedding.append(alpha*emb + (1-alpha) * embedding[ind[i+2]])
        ...
        # if two leftover objects: do linear
        elif np.remainder(len(object_set), 2) == 2:
            # compute new weighted mean
            alpha = compute_alpha(embedding[ind[-2]], embedding[ind[-1]], object_vector)

            nm, bv = best_weighted_mean(object_set[ind[-2]], object_set[ind[-1]], alpha, orig_set,
                                        dist_func, weighted_mean_func)

            new_object_set.append(nm)
            new_embedding.append(alpha * embedding[ind[-2]] + (1 - alpha) * embedding[ind[-1]])
