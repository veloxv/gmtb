...
def compute_median(object_set, weights, kernel_func, weighted_mean_func, rec_type,
                   kernel_matrix=None, dist_func=None, verbose=False):
    ...
    if type(weights) is str:
        if weights == "mean":
            weights = np.ones(n)
            dist = (np.sum(kernel_matrix) / np.power(n, 2) - 1/n * np.sum(kernel_matrix, 0)
                    - np.conjugate(1/n * np.sum(kernel_matrix, 0)) + np.diag(kernel_matrix))
        elif weights == "median":
        ...
        dist = np.sqrt(np.sum(weights @ np.transpose(weights) * kernel_matrix) / np.power(np.sum(weights, axis=0), 2)
                       - np.sum(weights * kernel_matrix, axis=0).T / np.sum(weights, axis=0)
                       - np.conjugate(np.sum(weights * kernel_matrix, axis=0).T / np.sum(weights, axis=0))
                       + np.diag(kernel_matrix))

    # reconstruction
    if rec_type == "linear":
        return gmtb.kernel.reconstruction.linear(object_set, weights, dist, kernel_func, dist_func,
                                                 weighted_mean_func, kernel_matrix, 2)
    elif rec_type == "triangular":
        return gmtb.kernel.reconstruction.linear(object_set, weights, dist, kernel_func, dist_func,
                                                 weighted_mean_func, kernel_matrix, 3)
    elif rec_type == "linear_recursive":
        return gmtb.kernel.reconstruction.linear_recursive(object_set, weights, dist, kernel_func, dist_func,
                                                           weighted_mean_func, kernel_matrix, verbose=verbose)
    elif rec_type == "triangular_recursive":
        return gmtb.kernel.reconstruction.triangular_recursive(object_set, weights, dist, kernel_func, dist_func,
                                                               weighted_mean_func, kernel_matrix, verbose=verbose)
    elif rec_type == "linear_search_linear_recursive":
        if verbose:
            print("Start linear_search_linear_recursive")
        obj, bv = gmtb.kernel.reconstruction.linear_recursive(object_set, weights, dist, kernel_func, dist_func,
                                                              weighted_mean_func, kernel_matrix, verbose=verbose)
        if verbose:
