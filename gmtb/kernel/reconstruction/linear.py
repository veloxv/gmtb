...
def linear(object_set, weights, dist, kernel_func, dist_func, weighted_mean_func, kernel_matrix, n_points):
    ...
    for i in range(iter):
        ...
        new_mean[i+1], k_mean_set, k_mean_mean, best_value_kernel_space[i+1] = \
            gmtb.kernel.best_weighted_mean(new_mean[i], k_mean_set, k_mean_mean,
                                           object_set[b], kernel_matrix[:,b], kernel_matrix[b,b],
                                           alpha, object_set, kernel_matrix, kernel_func,
                                           dist_func, weighted_mean_func)

    best_ind = np.argmin(best_value_kernel_space)
    return new_mean[best_ind], best_value_kernel_space[best_ind]